...
RECIPIENT="$RECIPIENT" CODE="$CODE" mix run --no-compile -e '
recipient = System.fetch_env!("RECIPIENT")
code = System.fetch_env!("CODE")

case AdventureTimeApi.Accounts.EmailDelivery.send_verification_code(recipient, code) do
  :ok ->
    IO.puts("Sent test verification email to #{recipient} with code #{code}")
