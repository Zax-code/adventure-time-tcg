...
export function Keypad({
  ...
  const keyRefs = useRef<Partial<Record<InteractiveKeyId, ElementRef<typeof View> | null>>>({});
  ...
  const findKeyAtPoint = useCallback((x: number, y: number) => {
    const layoutEntries = Object.entries(keyLayoutsRef.current) as Array<[InteractiveKeyId, KeyLayout]>;
    return layoutEntries.find(([, layout]) => {
      return x >= layout.x && x <= layout.x + layout.width && y >= layout.y && y <= layout.y + layout.height;
    })?.[0] ?? null;
  }, []);

  const activateKey = useCallback((keyId: InteractiveKeyId) => {
  ...
  const releaseTouches = useCallback((touches: Array<{ id: number }>) => {
    let changed = false;
  ...
  const handleTouchesUp = useCallback((ids: number[]) => {
    let changed = false;
    ids.forEach((id) => {
    ...
    Gesture.Manual()
      ...
      .onTouchesDown((event, manager) => {
        ...
        runOnJS(handleTouchesDown)(
          event.changedTouches.map(t => ({ id: t.id, x: t.absoluteX, y: t.absoluteY }))
        );
      })
      .onTouchesMove((event) => {
        'worklet';
  ...
  ), [handleTouchesDown, handleTouchesMove, handleTouchesUp, clearActiveTouches, touchCountSV]);

  return (
    <GestureDetector gesture={keypadGesture}>
      <View
        className="mx-4 mt-3 rounded-3xl border-2 border-primaryTint bg-white p-2"
        style={{
          shadowColor: "#000",
      ...
      >
        <View className="gap-2">
          {KEYPAD_ROWS.map((row) => (
            ...
            {row.map((key) => {
              ...
              const isDisabled = k === "DEL" ? keypadLocked || !answer : keypadLocked;
              const isPressed = activeKeys.includes(k);
              const isDigit = /^\d$/.test(k);
              const fontSizeClass = isDigit ? "text-[28px]" : k === "±" ? "text-[20px]" : "text-sm";
              ...
              return (
                <View
                  ...
                  style={{
                    ...
                    transform: [{ scale: isPressed && !isDisabled ? 0.95 : 1 }],
                  }}
                >
                  <Text className={keyTextClass}>
                    {label}
                  </Text>
                </View>
              );
        ...
        <View className="flex-row gap-2 mt-2">
          <View
            ...
            style={{
              opacity: keypadLocked || !answer ? 0.45 : activeKeys.includes("CLEAR") ? 0.7 : 1,
              transform: [{ scale: activeKeys.includes("CLEAR") && !keypadLocked && !!answer ? 0.95 : 1 }],
            }}
          >
          ...
          <View
            ...
            style={{
              opacity: submitDisabled ? 0.45 : activeKeys.includes("SUBMIT") ? 0.9 : 1,
              transform: [{ scale: activeKeys.includes("SUBMIT") && !submitDisabled ? 0.99 : 1 }],
            }}
          >
            <LinearGradient
