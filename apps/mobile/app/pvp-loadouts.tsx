...
export default function PvpLoadoutsScreen() {
  ...
  const selectedCards = useMemo(
    () => selectedCardIds.map((id) => cardMap.get(id)).filter(Boolean) as BuilderCard[],
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editingLoadout) {
        return apiClient.updatePvpLoadout(editingLoadout.id, loadoutName.trim(), selectedCardIds);
      }
      return apiClient.createPvpLoadout(loadoutName.trim(), selectedCardIds);
    },
    onSuccess: async () => {
      setToast({
        message: editingLoadout ? t("pvp.loadoutUpdated") : t("pvp.loadoutCreated"),
        type: "success",
      });
    ...
    onError: (error) => {
      setToast({ message: error instanceof Error ? error.message : t("messages.somethingWentWrong"), type: "error" });
    },
  });
  ...
  const toggleCardSelection = (card: BuilderCard) => {
    ...
    if (isSelected) {
      setSelectedCardIds((current) => current.filter((cardId) => cardId !== card.id));
      return;
    }
  ...
  const removeCard = (index: number) => {
    setSelectedCardIds((current) => current.filter((_, currentIndex) => currentIndex !== index));
  };
  ...
  return (
    ...
    >
      {toast ? (
        <View className="absolute left-4 right-4 z-50 items-center" style={{ top: insets.top + 12 }} pointerEvents="box-none">
          <View className={`w-full max-w-[420px] rounded-2xl px-4 py-3 ${toast.type === "success" ? "bg-successDark" : "bg-dangerDark"}`}>
            <Text className="text-center font-nunito-bold text-white">{toast.message}</Text>
          </View>
        </View>
      ) : null}

      <View className="border-b border-primaryTint bg-white/90 px-4 pb-4" style={{ paddingTop: insets.top + 8 }}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 32 }}>
        {loadouts.length > 0 ? (
          <View className="border-b border-primaryTint bg-white/60 px-4 py-4">
            <Text className="mb-2 font-nunito-semibold text-sm text-fgMuted">{t("pvp.yourLoadouts")}</Text>
            <ScrollView
              horizontal
            ...
            >
              {loadouts.map((loadout) => {
                ...
                return (
                  ...
                  >
                    <Text className={`font-nunito-semibold text-sm ${active ? "text-white" : "text-accentText"}`}>
                      {loadout.name}
                    </Text>
                  </Pressable>
                );
            ...
            {editingLoadout ? (
              <Pressable onPress={createNewLoadout} className="mt-4 rounded-xl bg-primaryDark px-4 py-2.5">
                <Text className="text-center font-nunito-bold text-xs text-white">{t("pvp.createLoadout")}</Text>
              </Pressable>
            ) : null}
          </View>
        ) : null}
        ...
        <View className="border-b border-primaryTint bg-white/80 px-4 py-4">
          <View className="flex-row items-center gap-2">
            ...
            />
            <Text className="font-nunito-semibold text-sm text-fgMuted">{selectedCards.length}/6</Text>
          </View>
          <Text className="mt-2 font-nunito text-xs text-fgMuted">
            {t("pvp.rarityCapLegendary", { count: rarityCounts.legendary })}, {t("pvp.rarityCapEpic", { count: rarityCounts.epic })}
          </Text>
              ...
              {[0, 1, 2].map((index) => {
                ...
                return (
                  ...
                  >
                    {card ? (
                      <>
                        <BuilderCardPressable className="h-full w-full" onLongPress={() => openCardDetails(card.id)}>
                          {card.imageAssetId ? (
                            <Image
                              source={{ uri: getCardImageUrl(card.imageAssetId), cacheKey: getCardImageCacheKey(card.imageAssetId) }}
                              contentFit="cover"
                        ...
                        <Pressable onPress={() => removeCard(index)} className="absolute -right-2 -top-2 rounded-full bg-dangerDark p-1">
                          <XIcon size={12} color="#fff" />
                        </Pressable>

                        <View className="absolute left-0 right-0 top-1 flex-row justify-between px-1">
                          {index > 0 ? (
                            <Pressable onPress={() => moveCard(index, "up")} className="rounded bg-black/50 px-1 py-0.5">
                              <Text className="font-nunito-bold text-[10px] text-white">{"<"}</Text>
                            </Pressable>
                          ) : (
                            <View />
                          )}
                          {index < selectedCards.length - 1 ? (
                            <Pressable onPress={() => moveCard(index, "down")} className="rounded bg-black/50 px-1 py-0.5">
                              <Text className="font-nunito-bold text-[10px] text-white">{">"}</Text>
                            </Pressable>
                          ) : null}
                        </View>
                        ...
                        <View className="absolute bottom-0 left-0 right-0 bg-black/60 px-1 py-1">
                          <Text className="font-nunito text-[10px] text-white" numberOfLines={1}>
                            {card.name}
                          </Text>
                        </View>
                      </>
          ...
          <Text className="font-nunito text-xs text-fgMuted">{t("pvp.activeBenchHint")}</Text>
          ...
          <View className="mt-4 flex-row gap-3">
            {editingLoadout ? (
              ...
              >
                <Text className="font-nunito-bold text-dangerDark">{t("common.delete")}</Text>
              </Pressable>
            ) : null}
            <Pressable onPress={() => setSelectedCardIds([])} className="flex-1 rounded-2xl bg-surfaceMuted px-4 py-3">
              <Text className="text-center font-nunito-bold text-fgMuted">{t("common.clear")}</Text>
            </Pressable>
            <Pressable
              disabled={selectedCardIds.length !== 6 || !loadoutName.trim() || saveMutation.isPending}
              onPress={saveLoadout}
            ...
            >
              ...
              >
                <Text className="text-center font-nunito-bold text-white">
                  {saveMutation.isPending ? t("admin.saving") : editingLoadout ? t("pvp.update") : t("pvp.create")}
                </Text>
              </LinearGradient>
            </Pressable>
          </View>
          ...
          >
            {LOADOUT_TYPES.map((type) => {
              ...
              return (
                ...
                >
                  <Text className={`font-nunito-semibold text-sm ${active ? "text-white" : "text-fgMuted"}`}>
        ...
        <View className="px-4 pb-4">
          <View className="flex-row flex-wrap justify-between gap-y-3">
            {filteredCollection.map((entry) => {
              ...
              const isSelected = selectedCardIds.includes(card.id);
              const rarity = RARITY_COLORS[card.rarity.name] ?? RARITY_COLORS.Common;
              ...
              return (
                <BuilderCardPressable
                  ...
                  style={{ width: "31.5%" }}
                >
                  <View className="overflow-hidden" style={{ aspectRatio: 3 / 4 }}>
                    {card.imageAssetId ? (
                      <Image
                        source={{ uri: getCardImageUrl(card.imageAssetId), cacheKey: getCardImageCacheKey(card.imageAssetId) }}
                        contentFit="cover"
                      ...
                      <View className="h-full w-full items-center justify-center" style={{ backgroundColor: typeColor.light }}>
                        <Text className="font-nunito-extrabold text-4xl" style={{ color: typeColor.dark }}>
                          {(card.character || card.name || "?").charAt(0)}
                        </Text>
                      </View>
                    )}
                  ...
                  <LinearGradient
                    ...
                    style={{ position: "absolute", left: 0, right: 0, bottom: 0, paddingHorizontal: 8, paddingVertical: 8 }}
                  >
                    <Text className="font-nunito text-xs text-white" numberOfLines={1}>
                      {card.name}
                    </Text>
                    <Text className="font-nunito text-[10px] text-white/80">
                      {t("pvp.hp")}:{card.hp} {t("pvp.atk")}:{card.attack} {t("pvp.def")}:{card.defense}
                    </Text>
                  </LinearGradient>
                  ...
                  {entry.quantity > 1 ? (
                    <View className="absolute left-2 top-2 rounded bg-black/60 px-1.5 py-0.5">
                      <Text className="font-nunito-bold text-[10px] text-white">x{entry.quantity}</Text>
                    </View>
                  ) : null}

                  {!isSelected ? (
                    <View className="absolute inset-0 border" style={{ borderColor: rarity.ring }} pointerEvents="none" />
                  ) : null}
                </BuilderCardPressable>
              );
            })}
          ...
          {filteredCollection.length === 0 ? (
            <View className="items-center px-6 py-12">
              <Text className="text-center font-nunito text-fgMuted">{t("pvp.noCardsFound")}</Text>
            </View>
          ) : null}
        </View>
      </ScrollView>
