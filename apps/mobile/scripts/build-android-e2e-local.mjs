...
const DEFAULT_PROFILE = "e2e-android";
const DEFAULT_OUTPUT_PATH = path.resolve(
  import.meta.dirname,
  "../local-build/android-e2e.apk",
);

function printHelp() {
  process.stdout.write(
    `Usage: npm run build:mobile:e2e:android -- [options]\n\nOptions:\n  --profile <name>   EAS build profile (default: ${DEFAULT_PROFILE})\n  --output <path>    Local APK output path (default: ${DEFAULT_OUTPUT_PATH})\n  --help             Show this help\n`,
  );
}
...
async function main() {
  ...
  if (!existsSync(options.outputPath)) {
    throw new Error(`Expected local Android E2E artifact at ${options.outputPath}.`);
  }

  process.stdout.write(`Local Android E2E build complete at ${options.outputPath}.\n`);
}
